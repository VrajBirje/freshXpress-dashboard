...
export default function FarmerDetailsContent({ farmer }: FarmerDetailsContentProps) {
  ...
  const updateVerificationStatus = async () => {
    setIsUpdating(true);
    try {
      const response = await fetch(`https://freshxpress-backend.onrender.com/api/farmers/${farmer.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        farmer.is_verify = !farmer.is_verify;
      }
    } catch (error) {
